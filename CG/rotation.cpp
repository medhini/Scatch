...
int ends[NENDS][2];
int coun=0; /* array of 2D points */
float x=0, y=0, r=15, g=9.8, u, ch1=0, ch2 = 0;
int theta;
int angle=0 ;//angle of rotation
float tof;
float X1[b1],Y1[b1],X2[b2],Y2[b2];
...
void repeated_draw()
{
    ...
    glLoadIdentity();
    //glTranslatef(25,14.3,0);
    glPushMatrix();
    glTranslatef(X1[coun],Y1[coun],0);
...
void calc1(int u, float theta)
{
    ...
    for(i=0; i<b1; i++)
    {
       X1[i] = u*cos(theta)*temp;
       Y1[i] = u*sin(theta)*temp - ((g*temp*temp)/2);
...
void calc2(int u, float theta)
{
    ...
    for(i=0; i<b2; i++)
    {
        X2[i] = u*cos(theta)*temp;
        Y2[i] = u*sin(theta)*temp - ((g*temp*temp)/2);
